...
# Set Up Chains
nchains=5
w = runif(nchains, 0.85,1)
beta_par = runif(nchains, 0.000025*10000, 0.00003*10000)
...
#write.csv(posterior, file="estim_1_Aug19.csv")
posterior <- read.csv(file="estim_1_Aug19.csv", header = TRUE)
...
summaries <- summaries %>% 
  mutate(across(where(is.numeric), ~ signif(.x, digits = 10)))
...
P <- ggplot(processed.long, aes(x = value, fill = variable, color = variable)) + 
  ...
  labs(title = "w=1", x = "Parameter Value", y = "Density") + 
