...
write.csv(posterior, file="estim_02_Sep1.csv")

#posterior <- read.csv(file="estim_02_Sep1.csv", header=TRUE)
...
P <- ggplot(processed.long, aes(x = value, fill = variable, color = variable)) + 
  ...
  labs(title = "w=0.5", x = "Parameter Value", y = "Density") + 
