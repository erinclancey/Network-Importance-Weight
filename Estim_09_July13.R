...
sim_dat %>%
  pomp(params=theta.start[3,]) -> make_prof
...
#write.csv(posterior, file="estim_09_July16.csv")
posterior <- read.csv(file="estim_09_July16.csv", header=TRUE)
...
P <- ggplot(processed.long, aes(x = value, fill = variable, color = variable)) + 
  ...
  labs(title = "w=0.9", x = "Parameter Value", y = "Density") + 
