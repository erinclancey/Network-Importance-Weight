...
source("Network Mech Model Pomp_SETUP.R")
registerDoParallel()
registerDoRNG(2488821)
#Make One dataset
day <- seq(1, 200, 1)
...
rw.var <- matrix(c(0.001,0,0,
                   0,0.001,0,
                   0,0,0.01), 
                 nrow = 3, dimnames = list(c("w","beta_par","k"), 
                                           c("w","beta_par","k")))
...
# Set Up Chains
nchains=5
w = runif(nchains, 0.2,0.7)
beta_par = runif(nchains, 0.00001*10000, 0.00004*10000)
...
posterior <- read.csv("Posterior_w_05.csv", header=TRUE)
posterior$beta_par <- posterior$beta_par/10000
###########################################################  
# Calculate acceptance rate for w
accepts <- posterior %>%
  dplyr::group_by(chain) %>%
  dplyr::summarise(
    accept_rate_w = mean(diff(w) != 0)
  )
accepts
####################################################
